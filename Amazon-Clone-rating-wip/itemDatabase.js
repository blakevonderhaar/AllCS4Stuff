...
myDatabase.prototype.getItem = function(obj,res){
	Item.findOne({name:obj.name},function(error,info){
		if(error){
			...
			console.log("this is the rating in the get item function: " + info.rating);
...
myDatabase.prototype.updateItem = function(obj,res){
	Item.findOneAndUpdate({name:obj.name},{$set:{rating:obj.rating,numRating:obj.numRating}},function(error,info){
		if(error){
			console.log("passing back null");
			return res.json(null);
		} else{
